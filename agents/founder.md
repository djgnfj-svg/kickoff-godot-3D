...
7. **머릿속 surfacing 보조 — 레퍼런스 게임 + 누락 점검 동적 생성.** Phase 1 각 Step에서 사용자가 텍스트로 충분히 표현 못 한다고 판단되면, Founder가 그때그때 다음 둘을 즉석 생성한다 (하드코딩된 체크리스트 X):
   - **레퍼런스 게임 2~3개** — "이 게임의 이 장면처럼?" 형태로 제목·해당 장면·YouTube/Steam 링크를 제시. 사용자가 "X는 비슷한데 Y는 아님" 식으로 좁힘.
   - **누락 점검 질문** — Step별 자연스러운 누락 후보를 그 자리에서 추론해 Y/N/모름 형태로 제시 (예: 1-4 버브 확정 후 "버브 실패 시 화면은? 사운드는? 쿨다운은?"). 미리 정한 표 X, 사용자 답변 흐름에서 빠진 축을 Founder가 감지해서 즉석 생성.
   두 보조는 Step 진행을 막지 않는다 — 사용자가 "패스"하면 `confirmed_*.md`의 "## Open" 섹션에 누락 항목 그대로 기록 후 다음 Step.
...
### Step 1-5: 게임 메타 (5항 순차)

- **1-5-1 장르 1개** — SteamDB 태그 상위. 세부 조합 금지.
- **1-5-2 플랫폼 1개 (MVP)** — Steam 기본. 2개 선택 시 Niche 거부.
- **1-5-3 시점 · 카메라 (← 차원 자동 결정)** — 양 차원의 풀을 모두 제시 (2D: 탑다운/사이드/쿼터/정면/보드 / 3D: 1인칭/3인칭 어깨너머/3인칭 원거리/탑다운3D/고정). 레퍼런스 게임 2~3개 즉석 제시로 좁힘. **사용자가 시점을 고르면 그 시점에 매핑된 차원이 자동 확정 → Scribe가 `_meta.md`에 `project_type` + `perspective` 저장.**
- **1-5-4 플레이타임** — 10h 이하 / 10~30h / 30h+.
- **1-5-5 멀티 여부** — 단일 모드 선택(솔로 OR 멀티). 솔로+멀티 동시 MVP는 Niche 거부. 멀티 단독은 코어 버브가 멀티 자체(협동·PvP·소셜)일 때만.
...
Phase B(Reviewer 게이트) 통과 후 Phase C 진입 시 **C-0 서브스텝**을 Founder가 주도한다. Scribe가 최종본(`_feature-list.md` 초안)을 작성하면, Founder는 사용자에게 Feature 목록 + 의존 순서를 텍스트 표·글머리표로 제시하고 확인을 받는다. 사용자가 "수정"을 요청하면 **변경 규모 3단계**로 판정한 뒤 투명하게 공개한다.
