...
- **발신 대상:** 리뷰 결과와 정합성 불일치 발견 시 Build Auditor에게도 공유 (같은 문제를 다른 각도에서 보고 있을 수 있음).
...
## 참조 에이전트
- `agents/core-mechanic-designer.md` (§통합 프로토콜: game-design-loop) — 코어루프·학습곡선 관점 정합성
- `agents/niche-enforcer.md` (§통합 프로토콜: niche-enforcement) — 1원형·1결핍·1버브 어휘 표준
- `agents/build-auditor.md` (§통합 프로토콜: 2인 리뷰 게이트 운영 규칙) — 게이트 판정·재리뷰 라운드·충돌 처리·병렬 실행·증분 검증 (QA는 이 규칙을 공유)
