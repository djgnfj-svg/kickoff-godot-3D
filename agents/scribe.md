...
- `docs/features/_feature-list.md` — Feature 목록 확정 본 (구조: 마일스톤 중첩 본문 / 매핑 표 / 대화 원문 / 변경 이력)

**구조 원칙:** 마일스톤이 1차 헤더, 그 아래 들어가는 F들이 nested. 평면 표는 마지막 "마일스톤 × F 매핑" 한 장만 둔다 (한눈 요약용). F 단위 체크리스트도 별도 섹션 분리하지 말고 **각 마일스톤 본문 안에** 둔다.
...
**사전 게이트 (마일스톤 아님):** 빈 Godot 프로젝트에서 `godot --headless --import` 종료코드 0. 통과 못 하면 M0 진입 불가, 사용자 에스컬레이션.

---

## M0 — Walking Skeleton ({기간})

**완료 시 보이는 화면:**
{한 문단 — 사용자가 실제 화면에서 무엇을 보는지}
...
**확인 체크리스트:**
- [ ] 플레이어가 게임 공간에서 primitive로 보인다
- [ ] 코어 버브 입력 시 1회 동작 수행 (피드백 1개 이상)
- [ ] smoke 씬 `godot --headless` 실행 종료코드 0

**검증 / 체크포인트:** {있으면 명시, 없으면 생략}
...
**확인 체크리스트:** (각 F의 feature-spec §0-B에서 인용)
- [ ] {측정 가능한 항목 1}
- [ ] {측정 가능한 항목 2}
- [ ] {측정 가능한 항목 3}
...
| 경로 | 용도 | 도입 F |
|------|------|--------|
| scenes/main.tscn | 루트 씬 | M0 |
| scenes/player/player.tscn | 주인공 | F1 |
| scenes/__dev/smoke.tscn | smoke (headless 테스트) | M0 |
...
| 경로 | 용도 | 도입 F |
|------|------|--------|
| scripts/autoload/game_state.gd | 씬 전환·세이브 | M0 |
...
```
M0: + main.tscn + smoke.tscn + GameState Autoload + Physics Layer 4종
...
게임 제작에서 **플레이 핵심**(이동·전투·적)에만 집중하다 보면 **UI/흐름 Feature**가 누락되기 쉽다. Walking Skeleton(M0)도 메뉴 없이 바로 플레이로 들어가므로, 기획 단계에서 "이 F가 목록에 있는가 or 의도적으로 제외했는가"를 Scribe가 명시적으로 점검한다.
...
소스: `_feature-list.md` + `how.md` 기술 섹션 + 각 `F{N}/feature-spec.md`의 §0-A·§0-B·§7.4
...
## 0. 마일스톤 진행 원칙
각 마일스톤은 **동작 기준 + 자동 테스트 + 사용자 검수** 3축이 모두 통과해야 다음 마일스톤으로 전진. Walking Skeleton(M0) 이후 모든 F는 코어루프를 닫는 상태에서 추가된다.

## 1. 마일스톤 일람
{_feature-list.md의 표 그대로 인용 (M0 → F1 → ... → FN)}

## 2. 마일스톤 상세

### M0 — Walking Skeleton (PRD 인용 섹션)
...
### M0 — Walking Skeleton
**동작 기준 (한 줄):** {_feature-list.md 인용}
**자동 테스트:** {how.md M0 완료 조건}
**사용자 검수:** `docs/build/M0/USER_CHECK.md`에서 §0-B 체크 (없으면 _feature-list.md "확인 체크리스트" 인용)
...
(상세는 `docs/features/F1/feature-spec.md §7.4` 참조)
...
**길이 기준:** F 수에 비례. 각 F 섹션은 1페이지 내. §7.4 상세는 인용하지 않고 링크.
