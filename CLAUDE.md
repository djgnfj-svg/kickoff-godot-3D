...
| 2026-04-29 | **feature-spec 정밀도 강화 — §7.3 매핑 표 + §9.1 씬 트리 + §9.6 데이터 스키마.** 두 구멍 메움: (1) §7.3 신설 — §0-B ↔ AC 매핑 표 (사용자 전담 ≤2개), 자동 테스트 PASS인데 사용자 NO 시 어느 AC 손볼지 추적 가능. 기존 §7.3 Given/When/Then은 §7.4로 이동. (2) §9.1 강화 — 씬 트리 코드 블록 강제 (Node 타입+부모-자식+첨부 스크립트), 0개면 DRAFT. F1에서 정한 트리를 F2가 모르던 충돌 방지. (3) §9.6 신설 — 데이터 스키마(.tres 클래스·필드·기본값), 밸런싱 수치 코드 하드코딩 금지. AC에 등장한 수치는 모두 §9.6에 매핑 강제. 영향 9파일: feature-spec/SKILL.md(주 변경) + evaluator/generator/scribe/build-handoff/build-orchestrator/sprint-evaluation/kickoff-orchestrator/sprint-planning의 §7.3 → §7.4 일괄 갱신. sprint-planning 필수 섹션 표는 7.3·7.4·9.1·9.6 4행으로 분리. | skills/feature-spec/SKILL.md, agents/{evaluator,generator,scribe}.md, skills/{build-handoff,build-orchestrator,sprint-evaluation,kickoff-orchestrator,sprint-planning}/SKILL.md | 사용자 점검: "각 F가 정밀하고 꼼꼼하게 기획되는지". 분석 결과 2구멍 — Generator가 .tscn 짤 때 노드 트리 모름, 자동 테스트 vs 사용자 검수 매핑 부재. 둘 다 채우기로 결정 |
