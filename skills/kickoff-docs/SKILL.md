...
세 문서는 서로 다른 질문에 답한다. 섞이지 않게 한다. 본 하네스는 **3D 게임 전용**이며, 모든 문서의 첫 줄은 `**프로젝트 종류:** 3D 게임 (Godot 4)` 로 시작한다.
...
6. **프로젝트 종류 헤더 의무:** 모든 문서 첫 줄에 `**프로젝트 종류:** 3D 게임 (Godot 4)` 명시.
...
## 한 줄 카피
{플레이어 원형 + 코어 버브 + 감정적 결과 — 예: "중세 기사가 되어 베어낸다 — 한 타의 무게감으로 긴장을 해소한다"}
...
- **시점/카메라:** {1인칭 / 3인칭 숄더 / 3인칭 고정 / 쿼터뷰 / 자유}
...
| 시점 | {1인칭 / 3인칭 숄더 / 3인칭 고정 / 쿼터뷰 / 자유} | why/what과 일치 |
| 에디터 자동화 | Godot MCP (capability matrix 관리) | Generator/Evaluator 자동화 |
| 테스트 | GUT 또는 GdUnit4 | GDScript TDD |
| 빌드/CI | `godot --headless --export` | 무인 빌드 |
| 에셋 소스 | `asset-pipeline` 4단계 폴백 체인 | placeholder-first |
...
## Godot MCP 의존
- capability matrix: `${CLAUDE_PLUGIN_ROOT}/skills/godot-mcp-protocol/references/capability-matrix.md`
- 갭 보고: `docs/godot-mcp/gaps/`
- MCP 부재 시 fallback: `godot --headless` CLI → `.tscn`/`.gd` 텍스트 편집 → 사용자 위임
- `godot-mcp-protocol` 스킬 참조

## 에셋 정책
- **1차:** Godot primitive (placeholder)
- **2차:** 무료 라이브러리 (Kenney / Quaternius / Poly Haven / Godot Asset Library)
- **3차:** AI 생성 (capability matrix 확인 — 이미지/SFX 우선, 3D는 옵션)
- **4차:** 사용자 위임
...
**M0는 2단계 필수 (환경 검증 → Walking Skeleton).** M0a를 코드 0줄로 선행하여 MCP·엔진·블렌더 환경이 먼저 돌아가는지 확인한 뒤에만 M0b로 넘어간다.

| M | 기간 | 산출물 | 완료 조건 |
|---|-----|-------|----------|
| **M0a** | 0.5~1일 | MCP 환경 검증 (코드 0줄) | ① Godot MCP로 에디터 창 열림 확인 ② Blender MCP로 블렌더 창 열림 확인 ③ 빈 Godot 프로젝트 생성 + `godot --headless --import` 성공 ④ Smoke 씬(빈 3D 씬) 실행 → 창 뜸 + 에러 로그 0 |
| **M0b** | 1~2주 | Walking Skeleton | 플레이어가 3D 공간에서 **코어 버브 1회** 수행 가능 (primitive 에셋, smoke 씬 headless 통과) |
| **M1** | ... | 코어루프 닫힘 | 4단계 루프 한 사이클 완주 + TTFV 3분 이내 + headless smoke + 60fps 유지 |
| **M2** | ... | F{N} 완성 | 해당 Feature 수용 기준 전부 통과 + 플레이테스트 시나리오 3개 통과 |

**M0a가 실패하면 M0b 진입 불가.** MCP 환경이 안 돌면 코드 작성해도 검증 경로가 없다. 환경 문제는 `docs/godot-mcp/gaps/G{N}.md`에 갭 리포트 작성 후 사용자 에스컬레이션.

## 테스트 전략
- **Smoke (필수):** `godot --headless --quit-after 5 scenes/__dev/smoke.tscn` 종료코드 0 + 에러 로그 0
- **단위 (GUT/GdUnit4):** 핵심 유틸·데미지 계산·상태 머신 전이
- **통합 (MCP 기반):** 씬 실행 → 입력 주입 → 프로퍼티 검증
- **시각 회귀:** 스크린샷 비교 (MCP 또는 사용자 위임)
...
- 3D 에셋 조달 (무료로 커버 가능한가, AI 3D 품질)
- MCP capability matrix 성숙도 (어디까지 자동화 가능한가)
