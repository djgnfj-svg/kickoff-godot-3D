...
**차원별 노드 타입·물리·렌더 디테일은 references 로드:** 2D → `references/2d.md`, 3D → `references/3d.md` (`docs/kickoff/_meta.md`의 `project_type`에 따라). 본 SKILL.md 본문은 차원-불변 규약(7항 표준·UID·의존성·핸드오프 양식)만 다룬다.
...
**원칙:** Autoload는 최소로. 남발 시 강결합 발생. 새 autoload 추가는 Kickoff how.md 근거 필요 (Phase B Build Auditor 검토).
...
### 7-1. 생성/수정되는 씬
| 경로 | 신규/수정 | 루트 노드 타입 (차원별) | 스크립트 | 용도 |
|------|-----------|------------------------|---------|------|
| scenes/systems/player.tscn | 신규 | CharacterBody2D 또는 CharacterBody3D (`references/{2d,3d}.md` 참조) | scripts/systems/player.gd | 플레이어 이동 |
...
루트 노드 타입과 자식 노드 구성은 차원에 따른다 — `references/2d.md` 또는 `references/3d.md`의 "물리 노드" / "루트·카메라·조명" 섹션 참조. 예시(3D):
```
...
|------|------|------|
| 씬 생성 | `.tscn` 텍스트 편집 | - |
| 스크립트 attach | `.tscn` 텍스트 편집 | - |
| Autoload 등록 | `project.godot` 텍스트 편집 | - |
| 시각 확인 | 사용자 위임 | Godot 에디터 직접 열기 |

### 7-3. 사용자 위임 항목
- {시각 확인이나 복잡 import 등 사용자에게 넘긴 작업 1줄 요약}
...
- **이미 할당된 UID는 건드리지 않는다.** Godot가 첫 import 시 자동 할당하면 그대로 둔다.
- **텍스트로 새 .tscn/.tres 파일을 만들 때**는 UID 없이 저장 가능 (Godot가 첫 로드 시 할당). 단 이미 다른 파일이 이걸 참조하면 경로로 먼저 참조하게.
- **파일 이동/이름 변경**은 가급적 Godot 에디터 내에서 수행 (참조 자동 갱신). 텍스트 편집으로 경로만 바꾸면 참조 끊어진다 — 사용자 위임 권장.
...
| 2026-04-29 | 차원별 노드·물리·렌더 디테일을 `references/{2d,3d}.md`로 분리. 본문은 차원-불변 7항 표준만 유지 | v0.2.0 — 2D/3D 겸용 지원. `_meta.md` `project_type`에 따라 reference 로드 |
