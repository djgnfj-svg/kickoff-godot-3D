...
└── references/
    ├── coding.md                     ← 폴더 구조·명명·언어 규칙·테스트 (공용)
    ├── design.md                     ← 레이아웃·타이포·색·상호작용 (공용 + 3D 기본값 포함)
    ├── accessibility.md              ← WCAG·키보드·스크린리더 (공용)
    ├── performance.md                ← 번들·응답시간·메모리·렌더링 (공용 + 3D 기본값 포함)
    ├── 2d.md                         ← 2D 차원 특화(픽셀 퍼펙트·canvas 정렬·draw call·atlas)
    └── 3d.md                         ← 3D 차원 특화(렌더러 선택·LOD·HDR·셰이더)
```

각 파일은 **플러그인이 제공하는 정적 레퍼런스**다. Godot 4 일반 규칙(GDScript 네이밍·노드 트리 패턴·Physics 등)이 미리 채워져 있다. Generator/Evaluator는 **읽기 전용**으로 사용한다.

### 프로젝트 차원에 따라 references 로드

Generator/Evaluator는 작업 시작 전 `docs/kickoff/_meta.md`의 `project_type`을 먼저 읽고, 다음 규칙에 따라 references를 로드한다:

- 항상 로드: `coding.md`, `design.md`, `accessibility.md`, `performance.md`
- `project_type = 2D 게임 (Godot 4)`이면 추가 로드: `references/2d.md` (3D 항목인 카메라 거리·LOD·HDR·셰이더 복잡도 규칙은 무시하고 2d.md의 대응 규칙으로 대체)
- `project_type = 3D 게임 (Godot 4)`이면 추가 로드: `references/3d.md` (이 파일이 design.md/performance.md의 3D 특화 항목을 보강)
...
## 게임 프로젝트 (Godot 4) 4도메인 초기 규칙

아래는 플러그인이 미리 채워둔 **Godot 4 일반 규칙 세트**다 (3D 기본값 포함, 2D는 `references/2d.md`로 대체). Generator/Evaluator는 읽기 전용으로 참조한다. 프로젝트별 특수 규칙(Autoload 이름·Physics Layer 번호·렌더러 등)은 `product-spec.md 7항`에 따로 기록한다.
