...
Feature의 유일한 "목표"는 **0-A 영상**과 **0-B 체크리스트** 두 층으로 기술한다. 0-A는 감성(플레이어 관점), 0-B는 검증(완성 판정). 정식 Given/When/Then은 §7.4에, §0-B↔AC 매핑은 §7.3에.
...
**Yes/No로 판단 가능한 짧은 문장 3~5개.** 이 체크리스트는 `_feature-list.md`에 **그대로 전시**되므로 짧고 명확하게.

- 각 항목은 한 줄. 기술 용어 허용 (금지 표현 규칙은 0-A 한정 — "WASD 입력", "3인칭 카메라 거리 4m" 같은 기술 표현 OK).
- 측정 가능해야 한다. "재미있다" ✗, "60초 프레임 드랍 0회" ✓.
- 최소 3개, 최대 5개. 그보다 상세한 수용 기준은 §7.4 Given/When/Then.
...
§0-B 각 항목이 어느 AC로 자동화되는지, 또는 사용자 전담 검수인지 1:1로 표시한다. 이 표는 Phase 2.7 USER_CHECK.md 생성과 Evaluator 자동 테스트의 연결고리다.

| §0-B 항목 | 자동 AC | 검증 방법 |
|----------|--------|----------|
| (§0-B 1번 그대로) | AC1 | GUT 타이머 / headless smoke / 좌표 assert 등 |
| (§0-B 2번) | AC2 | ... |
| (§0-B 3번) | — | **사용자 전담** (체감/시각/카메라 느낌 등 기계 판정 곤란) |

**규칙:**
- §0-B 모든 항목은 "AC 번호" 또는 "사용자 전담" 둘 중 하나로 매핑된다.
- "사용자 전담"은 §0-B 항목 중 **최대 2개**까지. 그 이상이면 자동화 가능 항목으로 분해 시도.
- 매핑 없는 §0-B 항목이 있으면 DRAFT 표시.

### 7.4 수용 기준 (Given / When / Then)
- **AC1.** Given ..., When ..., Then ...
- **AC2.** ...
- **AC3.** ...

§7.3 표의 "AC 번호" 열에서 참조되는 AC를 모두 정의한다. 최소 3개.
...
대상 `.tscn`마다 노드 트리를 코드 블록으로 명시. **Node 타입 + 부모-자식 관계 + 첨부 스크립트/리소스**까지. 속성 값은 §9.5 상태 전이 또는 §9.6 데이터 스키마에 분리.

예시 (3D F1 플레이어 이동):
```
scenes/player/player.tscn
Player (CharacterBody3D)              scripts/player/player.gd
├─ MeshInstance3D                      mesh = CapsuleMesh
...
├─ Sprite2D                            texture = res://assets/player.png
├─ CollisionShape2D                    shape = RectangleShape2D
└─ Camera2D                            zoom = (2, 2)
```

**검증:**
- §9.1에 트리 코드 블록이 0개면 DRAFT.
- 새 .tscn 파일 1개당 트리 1개 필수.
- 이미 다른 F에서 정의된 트리를 확장하는 경우: "F{M} §9.1 확장: `+ NodeName (NodeType)`" 형식으로 추가 노드만 표기.
- 트리에 등장한 모든 노드 타입은 Godot 4 내장 타입이어야 함 (커스텀 클래스는 §9.2에서 별도 명시).

### 9.2 Autoload / 싱글톤 / 커스텀 클래스
- Autoload (project.godot 등록): `GameState`, ...
- 커스텀 class_name: `class_name Hitbox extends Area3D` 등

### 9.3 Physics Layers
- 이 Feature가 사용하는 Layer 번호 (project.godot의 `physics/3d/layer_names` 기준): Layer 2 = Player, Layer 3 = Enemy ...
...
### 9.6 데이터 스키마 (Resource / .tres)

이 Feature가 사용·생성하는 `.tres` 또는 커스텀 `Resource` 클래스의 필드 스키마. 밸런싱 수치(데미지·HP·속도·쿨다운)는 코드 하드코딩 금지, 모두 여기.

| 리소스 클래스 | 경로 | 필드 | 타입 | 기본값 | 설명 |
|-------------|-----|-----|------|-------|-----|
| `PlayerStats` | `data/player/default.tres` | `move_speed` | float | 5.0 | m/s, 3D 기준 |
| | | `jump_velocity` | float | 4.5 | |
| `WeaponData` | `data/weapons/sword.tres` | `damage` | int | 10 | |
| | | `cooldown` | float | 0.4 | sec |

**검증:** 밸런싱 수치가 §7.4 AC에 등장하면 §9.6에 같은 필드가 있어야 한다. 없으면 DRAFT.
스키마가 없는 Feature(순수 입력 처리 등)는 "§9.6: 해당 없음"으로 명시.
...
- [ ] **TTFV 기여도 명시** (§5)
- [ ] **3층 훅 위치 명시** (§6)
- [ ] **§0-B ↔ AC 매핑 표 채워짐, 사용자 전담 ≤2개** (§7.3)
- [ ] 수용 기준이 Given/When/Then 형태로 **최소 3개** (§7.4)
- [ ] 실패 모드 최소 2개 (§12)
- [ ] **Pre-mortem 부정 리뷰 1개 이상** (§13)
- [ ] 관측 지표 최소 1개 (§11)
- [ ] **씬 트리 코드 블록 1개 이상** (§9.1)
- [ ] **밸런싱 수치(AC에 등장한 것)는 모두 §9.6 스키마에 매핑** 또는 "해당 없음" 명시
