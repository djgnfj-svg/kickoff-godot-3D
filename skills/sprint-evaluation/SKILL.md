...
**FAIL 사유 기록:**
- "무엇이" — 관찰된 결함 (구체. "캐릭터가 이상함" 금지, "Space 입력 시 attack 시그널 미발생" O)
- "어디서" — 파일:줄 또는 시나리오 이름
...
- **주관 판단:** "느낌이 둔하다" → 금지. design.md 규칙이 없으면 판정 불가, Phase 2.7로 위임 또는 규칙 추가 제안만
...
위 절차는 Godot 4 게임(2D/3D) 프로젝트 전용. 자동 재현은 `godot --headless` CLI + 텍스트 검증, 시각 확인은 Phase 2.7 USER_CHECK로 위임.
