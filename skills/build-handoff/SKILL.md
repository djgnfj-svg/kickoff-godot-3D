...
├── _roadmap.html                    # Visual Gate 로드맵 영구 보관 (참고용)
...
**"완료"의 정의:**
1. 모든 AC가 수동 재현 가능
2. {이 스프린트 특유의 "작동 증거" — 예: 특정 flow를 E2E로 실행 가능}
3. 관련 컨벤션 위반 0건
...
- 타입체크: `{명령어}` → 0건
- 테스트: `{명령어}` → 통과
- (UI 스프린트일 때) Playwright 시나리오: `{시나리오 이름 또는 경로}`
...
- S{M}-generator.md에 "7. Godot 산출 요약" (씬 트리, 사용한 MCP 경로, 발생한 갭 리포트, smoke 실행 결과) 추가
- `handoffs/S{M}-context.md`에 "씬 의존성 그래프" 추가

게임 프로젝트의 Planner/Generator/Evaluator는 **`build-handoff` + `godot-scene-handoff` 두 스킬을 모두 로드**한 뒤 작업한다. Godot MCP 사용 시에는 `godot-mcp-protocol`도 함께 로드.
...
### 파일: `docs/features/_roadmap.html` (Kickoff C-0 Visual Gate 영구 보관)

Scribe가 C-0에서 Visual Gate로 띄운 로드맵을 영구 저장한 HTML. Build 단계에서 사용자가 "현재 이 F 구현 중"을 시각으로 참조할 수 있다. Build 에이전트는 **수정하지 않음** (Kickoff 전용 자료).
