...
└── F{N}/feature-spec.md             # F별 상세 (§0-A·§0-B·§7.4·§9 등)
...
## 비가시 F 예외 (해당 시만 체크)
- [ ] 이 F의 §0-B가 전부 순수 내부 시스템이며 §7.4 AC와 1:1 매칭되어
...
- 자동 테스트(§7.4 AC) 깨뜨리지 말 것 — Evaluator가 재확인
...
**Evaluator 재확인 원칙:** 사용자 피드백 반영 후 Evaluator를 다시 돌려 §7.4 AC가 여전히 PASS임을 검증. 체감 조정이 기존 자동 테스트를 깨면 즉시 FAIL → 재설계 필요.
