...
3. **고수준 설계 결정**을 야심차게 채움 — "주인공은 회색 캡슐 placeholder, 카메라는 거리 4m 어깨너머, HUD는 좌상단 체력바" 수준까지 내려간다. 선언 없이 Generator에게 맡기면 평범한 결과
...
2. AC를 "작동하는 유저 가치" 단위로 묶음. 한 묶음 = 한 스프린트
   - 좋은 묶음: "코어 버브 1회 수행 + 피드백 1개 발생 (입력 → 액션 → 시각/사운드 피드백)"
   - 나쁜 묶음: "Player 씬만 만들기", "InputMap만 등록" — 플레이어가 볼 게 없음
...
2. 해당 스프린트 특유의 "작동 증거" — 예: "회색 평면에서 캐릭터가 입력대로 이동하고 카메라가 따라온다"
3. 컨벤션 위반 0건 (FAIL 규칙 기준)

**"자동 검증 훅"을 쓰는 법:**
- Godot 4 CLI 명령으로 기술. 명령어 셋업이 없으면 Generator가 M0a/첫 스프린트에서 스캐폴딩
- 기본 세트: `godot --headless --import`, `godot --headless --check-only`, GUT/GdUnit4 테스트, smoke 씬 헤드리스 실행
- 시각 확인이 필요한 항목은 자동 검증 훅에 넣지 말고 Phase 2.7 USER_CHECK로 위임
...
### Kickoff FROZEN 입력 계약 (읽기 전용)
Planner는 작업 시작 시 다음 파일을 가장 먼저 읽고 결정 분기:
- `docs/kickoff/_meta.md` — `project_type` (`2D 게임 (Godot 4)` / `3D 게임 (Godot 4)`)을 확인. 없으면 "Phase 0-1 먼저" 에러 후 중단.
- `docs/features/_feature-list.md` — Status=FROZEN 확인. 기술 통합 리스트(씬·스크립트·Autoload·Physics Layers·InputMap)는 입력 계약. 새 항목 추가 가능, 기존 삭제·이동 금지.

### 필수 선행 로드 스킬
Planner 세션 시작 시 `build-handoff` 외에 **반드시 로드**:
- `godot-scene-handoff` — product-spec.md 7항 / S{M}-plan.md 7항 형식
- `asset-pipeline` — 에셋 스프린트 경계 결정 (2단계 폴백)
- `build-conventions` 게임 섹션 — 4도메인 초기 규칙 채우기 (+ project_type에 따라 `references/{2d,3d}.md` 추가 로드)
...
### Step 3.5. 에셋 요구사항 추출 (2단계 폴백)

**각 스프린트마다** feature-spec의 AC와 씬 구성에서 에셋 요구사항을 추출하여 S{M}-plan.md에 **에셋 태스크** 섹션을 반드시 포함한다.

**추출 절차:**
1. AC를 읽으며 메시/스프라이트/머티리얼/텍스처/오디오/UI 에셋이 필요한 항목 식별 (3D면 메시/머티리얼, 2D면 스프라이트/AtlasTexture)
2. 각 에셋에 2단계 폴백 중 어느 단계를 쓸지 결정: `primitive`(Godot 내장 primitive/ColorRect/Sprite2D placeholder) / `user`(사용자가 외부에서 가져옴 — manifest.md에 출처·라이선스 기록)
...
| ID | 에셋 | 용도 | 폴백 단계 | 비고 |
|----|------|------|-----------|------|
| A1 | 플레이어 메시 | Player 씬 | primitive | 3D=CSGCapsule3D / 2D=Sprite2D placeholder |
| A2 | 바닥 타일 | World 씬 | user | 사용자가 가져옴, manifest 기록 |
```

`필요 도구` 컬럼은 두지 않는다. Generator는 `.tscn`/`.gd`/`.tres` 텍스트 직접 편집과 `godot --headless` smoke를 기본으로 사용하며, 시각 확인이 필요하면 사용자 위임으로 명시한다.
...
- [ ] 에셋 의존이 있는 스프린트는 `asset-pipeline` 2단계 중 어느 단계를 쓸지 명시
- [ ] 각 S{M}-plan.md에 "에셋 태스크" 테이블이 존재 (에셋 불필요 시 "없음" 명시)
- [ ] `_meta.md`의 project_type을 입력 계약으로 읽었음을 product-spec/plan에 명시
