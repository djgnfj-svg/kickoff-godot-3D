...
- `docs/kickoff/GDD.md` (Phase D-1 합본, 외부 공유용 비전 — 파생 뷰)
- `docs/kickoff/PRD.md` (Phase D-2 합본, 구현 진입점 마일스톤 walkthrough — 파생 뷰)
...
**프로젝트 고정 가정:** 엔진은 Godot 4로 고정. 차원(2D/3D)은 **Phase 1 Step 1-5-3 시점 선택의 부산물로 자동 결정**된다 (별도 사전 질문 없음).

차원 결정 시점:
- Step 1-5-3에서 사용자가 시점을 고르면 그 시점에 매핑된 차원이 확정됨
  - **2D 매핑:** 탑다운 2D / 사이드뷰 2D / 쿼터뷰 2D / 정면 2D / 보드뷰
  - **3D 매핑:** 1인칭 / 3인칭 어깨너머 / 3인칭 원거리 / 탑다운 3D / 고정 카메라
- Step 1-5-3 확정 즉시 Scribe가 `_meta.md`에 `project_type: {2d|3d}` 저장
- Phase A 진입 게이트: `_meta.md`에 `project_type` 존재 확인 (없으면 Step 1-5-3로 되돌림)

**Phase 1 Step 1-1 ~ 1-4는 차원 무관**으로 진행 (장르·메커닉·테마·포맷 키워드는 차원-비특이적). 차원-특화 references(`{2d,3d}.md`) 로드는 Step 1-5-3 이후에만 발생.
...
**1-5-3. 시점 · 카메라** — Founder가 차원별 옵션 풀에서 4~5개 보기 + 각 옵션의 레퍼런스 게임 2~3개(YouTube/Steam 링크) 즉석 제시 → 사용자 선택.

**시점 옵션** — 사용자가 차원을 사전에 결정하지 않았으므로 Founder는 **양 차원의 풀을 모두 제시**하고, 사용자가 선택한 시점에 따라 차원이 **자동으로 결정**된다. 시점 선택 직후 Scribe가 `docs/kickoff/_meta.md`에 `project_type: {2d|3d}` 저장.
...
```yaml
project_type: {2d|3d}
perspective: {선택한 시점 키}
engine: godot-4
decided_at: {timestamp}
```

레퍼런스 게임 2~3개를 즉석으로 보여주며 좁힌다. 시점 결정 → 차원 결정 → `_meta.md` 작성이 한 턴 안에 연쇄.
**1-5-4. 예상 플레이 시간** — 10h 이하 / 10~30h / 30h+ 중 1개.
**1-5-5. 멀티 여부** — 단일 모드 선택(솔로 OR 멀티). 솔로+멀티 동시 MVP는 Niche 거부. 멀티 단독 MVP는 코어 버브가 멀티 자체(협동·PvP·소셜 디덕션 등)일 때만 허용.

각 항목 대화 형태. 리서치 결과가 결정을 **거의 내려줬으면** Founder가 "결핍이 짧은 세션이었으니 10h 이하가 자연스러운데 확인?" 식으로 일관성 점검.

**저장:** `_workspace/game_meta.md` (1-5-1 ~ 1-5-5 각각 확정 요약, 1-5-3은 선택한 시점 + 레퍼런스 게임 링크 포함).
...
- 순서 + 의존
- M0(Walking Skeleton) 행을 **항상 먼저** 포함 (사전 게이트 `godot --headless --import` 통과는 마일스톤 아닌 사전 조건)
...
FROZEN된 `_feature-list.md` 순서대로 F1, F2, ... 순번 확정. 마일스톤(M0)은 feature-spec 불필요 (how.md 마일스톤 표로 충분). 각 F{N}마다:
...
## Phase D: 합본 컴파일 + 최종 정리 + 피드백

### D-0. 5종 단일 진실원 검증 (선행 조건)

GDD/PRD 컴파일 전에 5종이 모두 일관 상태인지 검증:

- `docs/kickoff/why.md`, `what.md`, `how.md` 첫 줄에 `_meta.md` `project_type` 값을 반영한 `**프로젝트 종류:** {2D|3D} 게임 (Godot 4)` 존재
- why의 플레이어 원형 = what의 대상 = how의 타깃 플랫폼/입력 가정인가
- what의 F1이 코어루프를 닫는 스프린트로 매핑 가능한가
- how의 **M0 사전 게이트 = `godot --headless --import` 종료코드 0** 명시 + **M0 완료 조건이 코어 버브 1회 수행** 둘 다 포함
- `docs/kickoff/reviews/` 2개 파일(build-auditor·qa) 전원 `APPROVED`
- `docs/features/_feature-list.md`의 Status가 `FROZEN`
- `docs/features/F*/feature-spec.md` 수 = `_feature-list.md`의 F 행 수 (마일스톤 M0 제외)
...
위 항목 중 하나라도 실패 시 D-1/D-2 진입 금지 → 해당 단계로 롤백.

### D-1. GDD.md 컴파일 (외부 공유용 비전 합본)

Scribe가 5종 + `_workspace/confirmed_*.md` + `_workspace/sellability_memo.md` + `_workspace/hook_memo.md`를 읽어 `docs/kickoff/GDD.md` 작성. **템플릿은 `agents/scribe.md`의 "Phase D 합본 산출물 → GDD.md" 섹션 참조.**

- 길이 5~10페이지
- 0~6번 섹션 모두 채움 (한 줄 요약 / 원형·결핍·버브 / 코어루프 / 3층 훅 / 게임 메타 / 시장·가격 / 출처)
- 헤더에 **"파생 뷰 — 직접 편집 금지"** 명시
- 첫 줄 `**프로젝트 종류:**` 헤더 의무 (`_meta.md` 참조)

### D-2. PRD.md 컴파일 (구현 진입점, 마일스톤 walkthrough)
...
- `docs/kickoff/GDD.md` + `docs/kickoff/PRD.md` 둘 다 존재
- 두 파일 모두 첫 줄 `**프로젝트 종류:**` 헤더 + "파생 뷰" 경고 헤더 포함
- GDD 길이 5~10페이지, PRD는 F 수 비례 (각 F 섹션 1페이지 내)
- 두 파일의 mtime이 5종 중 가장 최근 mtime 이후인가 (drift 감지)
- PRD의 마일스톤 표 행 수 = `_feature-list.md` 행 수 (M0 포함)

### D-4. 사용자 보고 + 피드백

1. 산출물 요약 3~5줄 보고. **GDD.md/PRD.md 경로 우선 노출** (외부 공유·구현 진입점). 5종 단일 진실원은 부가 언급.
2. 피드백 요청: "GDD/PRD에서 보강하고 싶은 부분이 있나요? (둘은 파생 뷰라 5종 수정 후 재컴파일됩니다.)"
3. 피드백 있으면:
   - GDD/PRD 자체 수정 요청 → 5종 어디 갱신해야 하는지 Founder가 판단 후 해당 위치 수정 → Scribe 재컴파일
   - 단순 표현·압축 강도 변경 → Scribe가 GDD/PRD만 재컴파일 (5종 무변경)
4. 피드백 없으면 종료.
