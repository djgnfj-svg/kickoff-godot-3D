...
| 축 | 검증 방법 | 임계값 |
|----|-----------|--------|
| **기능 완성도 (AC)** | 각 AC의 Given/When/Then 수동 재현 + Playwright (UI) 또는 API 호출 | 100% 통과 |
| **빌드/타입** | S{M}-plan의 "자동 검증 훅" 명령 실행 | 종료코드 0, 오류 0건 |
...
3. Playwright MCP가 필요한 AC가 있으면 시나리오 실행 + 스크린샷 저장
4. 각 AC를 수동 재현 절차로 점검 (S{M}-generator.md "AC 매핑"의 재현 방법 그대로)
...
- Playwright MCP 연결 실패: UI 축만 "검증 불가"로 표기, 나머지 축은 평가 진행. 결과는 `CONDITIONAL FAIL` — UI 재검증 후 최종 판정
...
- `godot-mcp-protocol`, `godot-scene-handoff`, `build-conventions` 게임 섹션, `game-design-loop`
