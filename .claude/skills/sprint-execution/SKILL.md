...
3. `.claude/skills/build-conventions/references/*.md` 4종 — 구현 중 지켜야 할 규칙
...
1. `필요 도구`에 "Godot MCP 필요"가 있으면:
   ```bash
   bash .claude/scripts/ensure-running.sh godot
   ```
2. `필요 도구`에 "Blender 실행 필요"가 있으면:
   ```bash
   bash .claude/scripts/ensure-running.sh blender
   ```
3. 둘 다 있으면:
   ```bash
   bash .claude/scripts/ensure-running.sh both
   ```
4. 에셋 태스크가 "없음"만이면 — 스킵
