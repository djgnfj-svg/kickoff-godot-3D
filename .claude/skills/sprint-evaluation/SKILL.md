...
4. `.claude/skills/build-conventions/references/*.md` 4종
...
프로젝트가 Godot 4 게임이면 위 절차에 **Playwright MCP 대신 Godot MCP + headless CLI**를 사용한다.

### 필수 선행 로드 스킬
- `godot-scene-handoff` — S{M}-generator.md 7항 해독
- `godot-mcp-protocol` — MCP capability 확인 + fallback 독립 재현
...
### Step 3 (AC 수동 재현) — Godot 씬 실행
AC마다 재현 경로:
- **게임플레이 AC:** Godot MCP가 가능하면 씬 실행 + 입력 주입 + 프로퍼티 읽기. 불가하면 사용자에게 "해당 씬을 에디터에서 Run하고 X를 확인해달라"고 위임.
- **순수 로직 AC:** 단위 테스트로 커버되면 테스트 실행으로 증거 채택.
- **시각 AC:** MCP 스크린샷 또는 `godot --headless` + 추후 수동 검토 (사용자 위임).

### Step 4 (Playwright 대체) — Godot MCP 활용
`godot-mcp-protocol` 스킬의 capability matrix를 먼저 읽는다. 현재 matrix에 **지원되는** 것만 MCP로 시도:
- 씬 열기 / 노드 조회 / 프로퍼티 읽기 / 시그널 구독
- 스크린샷 캡처 (matrix에 있으면) → `docs/build/F{N}/sprints/screenshots/S{M}-AC{k}.png`

matrix에 없으면 Fallback:
- (A) `godot --headless` + GDScript에서 `get_viewport().get_texture().get_image().save_png(...)` 추가 로직
- (B) 텍스트 점검만 (씬 파일 구조 확인)
- (C) 사용자 위임

MCP 호출이 실패하거나 matrix에 없으면 `docs/godot-mcp/gaps/G{N}.md` 갱신 — **Evaluator도 갭 리포트 책임**.
...
S{M}-plan.md의 "에셋 태스크" 테이블과 S{M}-generator.md의 7-2 "MCP 경로 테이블"을 교차 검증한다.
...
3. **manifest 기록:** 외부 에셋(library/blender-mcp/user)은 `docs/build/F{N}/assets/manifest.md`에 출처·라이선스가 기록되어 있는지 확인
4. **MCP 경로 정합성:** Generator가 7-2에 "blender-mcp"로 기록한 에셋은 실제로 glTF/glb 파일이 존재하는지 확인
5. **에셋 태스크 누락:** plan에는 있는데 generator.md에 언급이 없는 에셋 = FAIL

**위반 시:**
- 에셋 미존재 또는 import 실패 → FAIL (하드 임계값)
- manifest 미기록 → FAIL (라이선스 추적성)
- MCP 경로 불일치 → WARN (체제 개선 제안)

### 갭 리포트 생성 책임 (Generator와 동일)
Evaluator가 MCP 사용 중 matrix에 없는 기능을 만나면 즉시 `docs/godot-mcp/gaps/G{N}.md` 기록. 이미 있으면 "재발견: Evaluator 세션 {날짜}" 추가.
