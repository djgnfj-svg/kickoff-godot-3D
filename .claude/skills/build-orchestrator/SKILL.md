...
- `subagent_type: "general-purpose"` — 빌트인 타입 사용 (에이전트 정의 파일은 `.claude/agents/`에 존재)
- `model: "opus"` — 필수
- `prompt`에 다음을 포함:
  1. 어떤 역할로 동작할지 (`.claude/agents/{name}.md`를 먼저 Read하라는 지시)
...
### 0-0. 프로젝트 종류 감지
- `docs/kickoff/why.md` 첫 줄에 `**프로젝트 종류:** 3D 게임 (Godot 4)` 또는 `**프로젝트 종류:** SaaS/도구` 명시되어 있으면 그걸 사용
- 없으면 `docs/features/F{N}/feature-spec.md` 상단 헤더 또는 사용자 요청 키워드("게임/Godot/3D/플레이어/씬") 기반 추정
- 게임 프로젝트면 **아래 "게임 프로젝트 확장"** 섹션을 Phase 1~3 내내 함께 적용
...
### 0-1. 사용자 요청 파싱
   ...
   - **병렬 모드 트리거**: "F1,F3 병렬", "병렬로", "동시에", "worktree" → 모드 = **병렬(parallel)**. Phase P로 직행 (아래 참조)
...
1. Planner 서브 에이전트 1회 호출:
   ...
   Agent(
     ...
     model: "opus",
     prompt: "너는 planner 에이전트다. 먼저 .claude/agents/planner.md를 Read로 읽어 역할·원칙을 내재화하고, .claude/skills/sprint-planning/SKILL.md도 Read로 로드한 뒤 그 절차를 따른다. 이번 대상: F{N}. 입력: docs/features/F{N}/feature-spec.md, docs/kickoff/{why,what,how}.md, 사용자 프로젝트 루트. 산출: docs/build/F{N}/product-spec.md (FINAL), docs/build/F{N}/sprints/S{M}-plan.md (M=1..N), build-conventions/references/*.md 채우기. 완료 후 스프린트 수와 각 파일 경로를 반환."
   )
   ```
...
Agent(
  subagent_type: "general-purpose",
  model: "opus",
  prompt: "너는 generator 에이전트다. 먼저 .claude/agents/generator.md를 Read로 읽고, .claude/skills/sprint-execution/SKILL.md도 Read로 로드한 뒤 그 절차를 따른다. 이번 대상: F{N}, S{M}, 회차 1. 입력 파일 순서: docs/build/F{N}/product-spec.md, docs/build/F{N}/sprints/S{M}-plan.md, .claude/skills/build-conventions/references/*.md 4종. 사용자 프로젝트 루트에 코드 작성 + docs/build/F{N}/sprints/S{M}-generator.md 작성 후 결과(COMPLETE/PARTIAL) + 파일 경로 반환."
)
```
...
Agent(
  subagent_type: "general-purpose",
  model: "opus",
  prompt: "너는 evaluator 에이전트다. 먼저 .claude/agents/evaluator.md를 Read로 읽고, .claude/skills/sprint-evaluation/SKILL.md도 Read로 로드한 뒤 그 절차를 따른다. 이번 대상: F{N}, S{M}, Generator 회차 1. 입력: product-spec.md, S{M}-plan.md, S{M}-generator.md, build-conventions/references/*.md, 사용자 프로젝트 루트. 독립 재현 후 S{M}-evaluation.md 작성, PASS/FAIL 결과 반환."
)
```
...
3. 마지막 스프린트면 Phase 3로, 아니면 Phase 2-1로 (M+1 스프린트)
...
Agent(
  ...
  prompt: "너는 이 저장소의 build-orchestrator를 F{N}에 대해 실행하는 독립 에이전트다. 먼저 .claude/skills/build-orchestrator/SKILL.md를 Read로 로드하고, 수동 모드 규칙에 따라 F{N}을 초기 실행부터 Phase 3까지 완수해라. 단 Phase 3에서 '다음 F'로 자동 진행하지 말고 final-context.md만 쓰고 종료해라. 에스컬레이션(Phase 2-6) 발생 시 즉시 종료하고 사유를 반환값에 담아라. 병렬 세션이므로 다른 F의 상태를 가정하지 말 것."
)
```
...
- .claude/skills/godot-scene-handoff/SKILL.md
- .claude/skills/godot-mcp-protocol/SKILL.md
...
4. PASS되면 `.claude/skills/godot-mcp-protocol/references/capability-matrix.md` 갱신 (사용자 승인)
