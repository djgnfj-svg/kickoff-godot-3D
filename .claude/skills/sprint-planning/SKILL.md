...
## 입력
- `docs/features/F{N}/feature-spec.md` — 주 입력
...
3. `build-conventions/references/{coding,design,accessibility,performance}.md` 각 파일의 규칙 채움
...
`build-conventions/references/{coding,design,accessibility,performance}.md` 각각:

**먼저 확인:** 이 도메인이 Feature 범위에 적용되는가?
- 프론트 없는 백엔드 Feature → design, accessibility = "해당 없음"으로 기록
- CLI 도구 → design, accessibility = "해당 없음"
- 스크립트 일회성 실행 → performance = 대부분 "해당 없음"

**해당되는 도메인만** 3-5개 규칙을 채운다:
1. 사용자 프로젝트의 기존 설정(`tsconfig.json`, 린터, 디자인 토큰 파일 등) 확인
2. 그 설정과 호환되는 규칙으로 작성
3. 각 규칙은 **반드시 "검증" 필드가 실행 가능**해야 한다. "검증: 수동 리뷰"는 무효

예시 채우기 흐름은 `build-conventions` 스킬의 references 파일 주석 참조.
